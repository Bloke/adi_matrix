...
/*
    ...
    2.1     - further updates for TXP 4.6
            - de-colon-isation
    beta4   - 4.7 fixes (paging)
    beta5   - fixes for glz_cfs v2.0 (much thanks to jakob)
    beta6   - STRICTly fix for criteria_status error in admin tab
    beta7   - fix function parameter order warning
...
if (txpinterface === 'admin') {
    global $adi_matrix_debug, $adi_matrix_dump, $adi_matrix_txp460, $adi_matrix_txp470;
    ...
    $adi_matrix_txp470 = (version_compare(txp_version,'4.7-dev','>='));
...
# --- END PLUGIN TEXTPACK ---
    ...
    $adi_matrix_glz_cfs_v20 = FALSE;
    if ($adi_matrix_glz_cfs) {
        $glz_cfs_version = safe_field("version", 'txp_plugin', 'name = "glz_custom_fields"');
        $adi_matrix_glz_cfs_v20 = (version_compare($glz_cfs_version, '1.9','>=')); // using 1.9 coz it don't recognise "2.0 beta" (no hyphen?)
    }
    if ($adi_matrix_debug) {
        echo "<b>glz_custom_fields:</b>".br;
        if ($adi_matrix_glz_cfs) {
            echo "version = $glz_cfs_version";
            if ($adi_matrix_glz_cfs_v20) echo " (v2.0)";
        }
        else
            echo "not installed";
        echo br;
...
// mostly ripped off from doArticles() in publish.php
    global $adi_matrix_debug,$adi_matrix_categories,$adi_matrix_nulldatetime;
    ...
    switch ($expiry) {
        case '1': // no expiry set
            $time .= " AND Expires = ".$adi_matrix_nulldatetime;
            break;
        case '2': // has expiry set
            $time .= " AND Expires != ".$adi_matrix_nulldatetime;
            break;
        case '3': // expired
            $time .= " AND now() > Expires AND Expires != ".$adi_matrix_nulldatetime;
            break;
    }
...
function adi_matrix_get_articles($matrix_index,$offset,$limit,$where='1',$sortq='') {
...
// translate $_POSTED article data into SQL-speak
    global $adi_matrix_debug,$txp_user,$adi_matrix_cfs,$adi_matrix_articles,$vars,$prefs,$adi_matrix_nulldatetime;
    ...
    if (isset($data['expires'])) {
        ...
        if ($expiry) {
            ...
            $expiresq = "Expires=".$adi_matrix_nulldatetime.", ";
    }
    // custom fields
    $cfq = array();
...
// default values for new article, adjusted for specified matrix
    global $adi_matrix_debug, $adi_matrix_list,$prefs, $adi_matrix_cfs, $adi_matrix_glz_cfs, $adi_matrix_glz_cfs_v20;
...
// update database with posted data or article defaults
    global $adi_matrix_debug,$adi_matrix_cfs,$txp_user,$prefs,$vars,$step,$adi_matrix_list,$adi_matrix_article_defaults,$adi_matrix_nulldatetime;
    ...
    foreach ($adi_matrix_cfs as $index => $cf_name) {
        ...
        if (array_key_exists($custom_x,$adi_matrix_list[$matrix_index])) // check that custom field is known to adi_matrix
            if (isset($data[$custom_x])) // present in data posted
                $$custom_x = $data[$custom_x];
            else // not present in posted data, so set default
    ...
    if ($expires)
        ...
        $whenexpires = $adi_matrix_nulldatetime;
...
// delete an article - ripped off from list_multi_edit() in txp_list.php
    ...
    if (!has_privs('article.delete')) {
        ...
        if (has_privs('article.delete.own'))
            $allowed = safe_column_num('ID', 'textpattern', 'ID in('.implode(',',$selected).') and AuthorID=\''.doSlash($txp_user).'\'',$adi_matrix_debug);
        $selected = $allowed;
    }
...
function adi_matrix_glz_cfs_input($custom_x,$var,$val,$id) {
// output custom field input according to glz_custom_fields format
    global $adi_matrix_debug, $adi_matrix_glz_cfs_v20;
...
// analyse submitted article data ($_POST), massage if necessary, & create new list of articles & data ($adi_matrix_post)
    ...
    if ($adi_matrix_glz_cfs) {
        if ($adi_matrix_debug) echo '<b>glz_cfs blanks generated for:</b>'.br;
    ...
    foreach ($adi_matrix_post as $id => $data) {
        foreach ($data as $field => $new_value) {
            ...
            if ($id == 'new') { // new article
                ...
                if ($adi_matrix_debug) echo 'id='.$id.', field='.$field.', new_value='.$new_value.' (NEW ARTICLE)'.br;
            }
            else { // existing article
                $equal = TRUE;
                $old_value = $adi_matrix_articles[$id][$field];
                $test_value = $new_value;
                if ($field == 'keywords') // remove human friendly spaces after commas
                    $test_value = str_replace(', ' ,',', $new_value);
                if (($field == 'posted') || ($field == 'expires')) // date/time requires special attention (because DB time may be different to TXP human time, so direct comparison not valid)
                    if (array_key_exists('reset_time',$new_value)) {
                        $equal = FALSE; // force inequality - "NOW()" time will be set in database update
                        if ($adi_matrix_debug) echo 'id='.$id.', field='.$field.' (RESET)';
                    }
                    else { // use article's DB unix timestamp & convert to TXP date/time for comparison
                        $ufield = 'u'.$field; // tweak field name from posted/expires to uposted/uexpires
                        if ($adi_matrix_articles[$id][$ufield])
                            $old_value = safe_strftime("%Y-%m-%d %X",$adi_matrix_articles[$id][$ufield]); // use DB unix timestamp & convert to TXP date/time
                        else
                            $old_value = '0000-00-00 00:00:00';
                        $test_value = $date_string; // use new time date/time string
                        if ($adi_matrix_debug) echo 'id='.$id.', field='.$field.', old_value='.$old_value.', test_value='.$test_value;
                    }
                else
                    if ($adi_matrix_debug) echo 'id='.$id.', field='.$field.', new_value='.$new_value.', old_value='.$old_value.', test_value='.$test_value;
...
function adi_matrix_validate_post_data($adi_matrix_articles,$post_data) {
// article data validation
    global $adi_matrix_debug,$adi_matrix_validation_errors;
    ...
    foreach ($post_data as $id => $data) {
        ...
        if (($url_title_count > 1) || (($id == 'new') && $url_title_count)) { // duplicates found (existing articles: multiple in DB, new article: one or more in DB)
            $msg = 2;
...
// plot in the title
    global $event,$step,$adi_matrix_cfs,$adi_matrix_list,$adi_matrix_glz_cfs,$adi_matrix_categories,$txp_user;
...
// - to fix would involve a custom elink() function to pass the 'page' variable on
    ...
    foreach ($field_list as $field) {
        ...
        else ; // no arrow - sort set in admin
    ...
    if ($sort == 'section') // sort value matches field
        ...
        $class = ' class= "section"'; // no arrow - sort set in admin
...
function adi_matrix_table($adi_matrix_articles,$matrix_index,$page,$errors=array(),$updates=array()) {
...
// td (field specific): adi_matrix_field_id, adi_matrix_field_title, adi_matrix_field_custom_x etc etc
    global $step,$adi_matrix_debug,$adi_matrix_cfs,$adi_matrix_statuses,$adi_matrix_list,$txp_user,$txp_user,$adi_matrix_glz_cfs,$adi_matrix_validation_errors,$adi_matrix_txp460;
    ...
    if ($adi_matrix_articles) {
        foreach ($adi_matrix_articles as $id => $data) {
            ...
            if ($adi_matrix_debug && ($step == 'update')) {
            ...
            if ($adi_matrix_list[$matrix_index]['title']) {
                $has_privs ? // decide if user gets input fields or not
                    $out .= tda(finput("text",$prefix."[title]",$data['title'],'',(adi_matrix_pref('adi_matrix_input_field_tooltips') ?htmlspecialchars($data['title']):'')).$arrow_link,' class="adi_matrix_field_title"') :
            ...
            if ($adi_matrix_list[$matrix_index]['section']) {
                $has_privs ? // decide if user gets input fields or not
                    $out .= tda(adi_matrix_section_popup($prefix."[section]",$data['section'],$adi_matrix_list[$matrix_index]['criteria_section']),' class="adi_matrix_field_section"') :
            ...
            if ($adi_matrix_list[$matrix_index]['status'])
                $has_privs ? // decide if user gets input fields or not
                    $out .= tda(selectInput($prefix.'[status]',$adi_matrix_statuses,$data['status']),' class="adi_matrix_field_status"') :
                    $out .= tda($adi_matrix_statuses[$data['status']],' class="adi_matrix_field_status"');
            // custom fields
            foreach ($adi_matrix_cfs as $index => $cf_name) {
                $custom_x = 'custom_'.$index;
                if (array_key_exists($custom_x,$adi_matrix_list[$matrix_index])) // check that custom field is known to adi_matrix
                    if ($adi_matrix_list[$matrix_index][$custom_x])
                        if ($has_privs) // decide if user gets input fields or not
                            if ($adi_matrix_glz_cfs) {
                                $glz_input_stuff = adi_matrix_glz_cfs_input($custom_x,$prefix."[$custom_x]",$data[$custom_x],$id);
                                ...
                                $out .= tda(finput("text",$prefix."[$custom_x]",$data[$custom_x],'',(adi_matrix_pref('adi_matrix_input_field_tooltips')?htmlspecialchars($data[$custom_x]):'')),' class="adi_matrix_field_'.$custom_x.'"');
                        else
...
//              }
                $has_privs ? // decide if user gets input fields or not
                    $out .= tda(finput("text",$prefix."[article_image]",$data['article_image'],'',(adi_matrix_pref('adi_matrix_input_field_tooltips')?htmlspecialchars($data['article_image']):'')).$arrow_link,' class="adi_matrix_field_image"') :
            ...
            if ($adi_matrix_list[$matrix_index]['category1'])
                $has_privs ? // decide if user gets input fields or not
                    $out .= tda(adi_matrix_category_popup($prefix."[category1]",$data['category1'],FALSE),' class="adi_matrix_category adi_matrix_field_category1"') :
            ...
            if ($adi_matrix_list[$matrix_index]['category2'])
                $has_privs ? // decide if user gets input fields or not
                    $out .= tda(adi_matrix_category_popup($prefix."[category2]",$data['category2'],FALSE),' class="adi_matrix_category adi_matrix_field_category2"') :
            ...
            if ($adi_matrix_list[$matrix_index]['posted'])
                $has_privs ? // decide if user gets input fields or not
                    $out .= tda(adi_matrix_timestamp_input($prefix."[posted]",$data['posted'],$data['uposted'],'posted'),' class="'.$class.'"') :
            ...
            if ($adi_matrix_list[$matrix_index]['expires'])
                $has_privs ? // decide if user gets input fields or not
                    $out .= tda(adi_matrix_timestamp_input($prefix."[expires]",$data['expires'],$data['uexpires'],'expires'),' class="'.$class.'"') :
...
// styles as per adi_matrix_table()
    global $adi_matrix_debug,$adi_matrix_cfs,$adi_matrix_statuses,$adi_matrix_list,$adi_matrix_glz_cfs,$adi_matrix_article_defaults;
    ...
    // status
    if ($adi_matrix_list[$matrix_index]['status'])
        $out .= tda(selectInput($prefix.'[status]',$adi_matrix_statuses,$defaults['status']),' class="adi_matrix_field_status"');
    // custom fields
    foreach ($adi_matrix_cfs as $index => $cf_name) {
        $custom_x = 'custom_'.$index;
        if (array_key_exists($custom_x,$adi_matrix_list[$matrix_index])) // check that custom field is known to adi_matrix
            if ($adi_matrix_list[$matrix_index][$custom_x])
                if ($adi_matrix_glz_cfs) {
                    $glz_input_stuff = adi_matrix_glz_cfs_input($custom_x,$prefix."[$custom_x]",$defaults[$custom_x],0);
                    if ($glz_input_stuff[1] == 'glz_custom_radio_field') // don't apply glz_class coz can't handle glz reset function properly yet - see below
                        $out .= tda($glz_input_stuff[0],' class="adi_matrix_field_'.$custom_x.'"');
                    else
                        $out .= tda($glz_input_stuff[0],' class="'.$glz_input_stuff[1].' adi_matrix_field_'.$custom_x.'"');
                    ...
                    $out .= tda(finput("text",$prefix."[$custom_x]",$defaults[$custom_x],'',(adi_matrix_pref('adi_matrix_input_field_tooltips')?htmlspecialchars($defaults[$custom_x]):'')),' class="adi_matrix_field_'.$custom_x.'"');
    }
    // article image
    if ($adi_matrix_list[$matrix_index]['article_image'])
    ...
    // category1
    if ($adi_matrix_list[$matrix_index]['category1'])
        $out .= tda(adi_matrix_category_popup($prefix."[category1]",$defaults['category1'],FALSE),' class="adi_matrix_category adi_matrix_field_category1"');
    // category2
    if ($adi_matrix_list[$matrix_index]['category2'])
        $out .= tda(adi_matrix_category_popup($prefix."[category2]",$defaults['category2'],FALSE),' class="adi_matrix_category adi_matrix_field_category2"');
    // posted
    if ($adi_matrix_list[$matrix_index]['posted'])
        $out .= tda(adi_matrix_timestamp_input($prefix."[posted]",$defaults['posted'],$defaults['uposted'],'posted'),' class="adi_matrix_timestamp adi_matrix_field_posted"');
    // expires
    if ($adi_matrix_list[$matrix_index]['expires'])
        $out .= tda(adi_matrix_timestamp_input($prefix."[expires]",$defaults['expires'],$defaults['uexpires'],'expires'),' class="adi_matrix_timestamp adi_matrix_field_expires"');
...
// a matrix tab
    global $prefs, $adi_matrix_debug, $adi_matrix_dump, $adi_matrix_list, $adi_matrix_articles, $adi_matrix_cfs, $adi_matrix_sort_dir, $adi_matrix_sort_type, $adi_matrix_sort_options, $adi_matrix_validation_errors, $adi_matrix_article_defaults, $adi_matrix_txp470;
    ...
    $where = adi_matrix_where($criteria);

    // article sort query
    $sortq = adi_matrix_order_by($sort,$dir,$sort_type);

    // paging Mr. Matrix
    if ($step == $event.'_change_pageby') { // change of page length
        $qty = gps('qty');
        adi_matrix_pref($event.'_pageby',$qty,TRUE);
    }
    $page = gps('page'); // get page number
    $pageby = get_pref($event.'_pageby',($adi_matrix_txp470 ? 12 : 15)); // get current page size (paging default, if not saved as pref)
    ...
    $adi_matrix_articles = adi_matrix_get_articles($matrix_index,$offset,$pageby,$where,$sortq);

    // article defaults
    $adi_matrix_article_defaults = adi_matrix_article_defaults($matrix_index);

    // $step aerobics
    if ($step == 'update') {
        $post_data = adi_matrix_get_post_data($adi_matrix_articles,$matrix_index);
        $errors = adi_matrix_validate_post_data($adi_matrix_articles,$post_data);
    ...
    $table = adi_matrix_table($adi_matrix_articles,$matrix_index,$page,$errors,$updates);
...
END_SCRIPT;
    ...
    echo tag(
            ...
            .($adi_matrix_txp470 ? Txp::get('\Textpattern\Admin\Paginator', $event, $step)->render($pageby) : pageby_form($event, $pageby))
            ,'div'
...
// check/perform upgrade
    ...
    // version 2.1
    $rs = safe_query('SHOW FIELDS FROM '.safe_pfx('adi_matrix')." LIKE 'ordinal'",$adi_matrix_debug);
    $a = nextRow($rs);
    $v2_1 = empty($a);
    ...
    if ($do_upgrade && $upgrade_required) {
        ...
        if ($v2_0) {
            ...
            $res = $res && safe_delete('txp_prefs',"name='adi_matrix_article_limit'",$adi_matrix_debug); // made redundant by paging
        }
        if ($v2_1) {
            $res = $res && safe_query("ALTER TABLE ".safe_pfx("adi_matrix")." ADD `ordinal` VARCHAR(32) DEFAULT 1 NULL AFTER `id`",$adi_matrix_debug);
        }
        return $res;
    }
...
// downgrade to previous version - 2.0 to 1.1/1.2 only
    ...
    $res = $res && safe_query("ALTER TABLE ".safe_pfx("adi_matrix")." DROP `ordinal`",$adi_matrix_debug);
...
function adi_matrix_timestamp_input($name,$datetime,$ts,$type='posted') {
...
// read or set pref
    ...
    if ($value === NULL) { // return pref value
        if (isset($adi_matrix_prefs[$name]['value'])) // may be dynamically generated pref
...
//      if (isset($adi_matrix_prefs[$name]['value'])) // may be dynamically generated pref
//          $prefs[$name] = get_pref($name,$adi_matrix_prefs[$name]['value'],TRUE);
//      else //??? 4.7 FRIG
